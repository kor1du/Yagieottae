...
public class AlramService
{
    ...
    public ResponseDto saveAlram(AlramDto.Request alramDto)
    {
        ...
        try
        {
            ...
            {
                if (alramRepository.findExsistingAlram(user.getId(), alramDto.getPillId()).isPresent())
                {
                    throw new CustomBadRequestException("기존에 설정해둔 알람이 존재합니다. 알람은 약 하나당 한개씩만 설정 가능합니다.");
                }
            } else
            {
                existingAlram = alramRepository.findById(alramDto.getId()).orElseThrow(() -> new CustomBadRequestException("기존 알람 정보가 존재하지 않습니다!"));
            }

            Pill pill = pillRepository.findById(alramDto.getPillId()).orElseThrow(() -> new CustomBadRequestException("저장하시려는 약을 찾을 수 없습니다.")); //pill의 pk값으로 조회
    ...
    //알람 목록을 가져온다
    public ResponseDto findAlrams(Boolean getToday)
    {
        ResponseDto returnValue;
        ...
        try
        {
            ...
            if (getToday) //오늘 날짜의 알람 목록들만 가져와야 한다면
            {
                ...
                alramList = alramRepository.findTodayAlrams(user.getId(), today).get();
            } else //현재 요일에 상관없이 알람 목록을 가져와야 한다면
            {
                alramList = alramRepository.findAllAlrams(user.getId()).get(); //전체 알람 목록을 DB에서 조회
            }
    ...
    public ResponseDto deleteAlram(Long alramId)
    {
        ...
        try
        {
            if (alramRepository.findById(alramId).isEmpty()) //삭제 대상 알람이 존재하지 않는다면
            {
