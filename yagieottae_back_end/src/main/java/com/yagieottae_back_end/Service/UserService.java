...
public class UserService
{
    ...
    public ResponseDto login(UserDto.Login userLoginDto)
    {
        ...
        try
        {
            User user = userRepository.findByUserId(userLoginDto.getUserId()).orElseThrow(() -> new CustomBadRequestException("해당 유저가 존재하지 않습니다!")); //유저ID로 유저 검색
            ...
            Authentication authentication = authenticationManagerBuilder.getObject().authenticate(authenticationToken); //authenticationToken 객체를 토대로 authentication 객체 생성
            ...
            redisTemplate.opsForValue() //redis에 refreshToken 정보 저장
                         .set("RT:" + authentication.getName(), //redis 키값
                                 jwtTokenDTO.getRefreshToken(), //redis 내용
                                 jwtTokenDTO.getRefreshTokenExpireMs().getTime(), //redis 만료시간
    ...
    public ResponseDto reissue(String refreshToken)
    {
        ...
        redisTemplate.opsForValue().set("RT:" + authentication.getName(), jwtTokenDTO.getRefreshToken(), jwtTokenDTO.getRefreshTokenExpireMs().getTime(), TimeUnit.MILLISECONDS); //새로 생성된 토큰정보에서 refreshToken 정보를 redis에 저장
    ...
    public ResponseDto signup(UserDto.Signup userSignupDto)
    {
        ...
        try
        {
            ...
            if (userRepository.findByUserId(userSignupDto.getUserId()).isPresent()) // 동일한 아이디로 이미 가입한 유저가 존재한다면
            {
                throw new CustomBadRequestException("입력하신 아이디와 중복되는 아이디가 존재합니다. 다른 아이디를 사용해주세요.");
            }

            if (userRepository.findByNickname(userSignupDto.getNickname()).isPresent()) //별명 중복체크
            {
            ...
            User user = User.builder() //DB에 저장할 User 객체 생성
                            ...
                            .regDate(new Date()).role(Role.USER)
                            .build();
    ...
    public Authentication getAuthenticationFromRedisByRefreshToken(String refreshToken)
    {
        ...
        String redisRefreshToken = redisTemplate.opsForValue().get("RT:" + returnValue.getName()); //authentication 변수에 저장되어있는 유저이름으로 redis에 저장된 값이 있는지 찾고 redis에 저장되어 있는 RefreshToken정보로 변수 초기화
     ...
     * 반환 값: 로그아웃 결과
     **/
    public ResponseDto logout(UserDto.Logout logoutDto)
    {
        ResponseDto returnValue;
        try
        {
            Authentication authentication = jwtTokenService.getAuthentication(logoutDto.accessToken); //logoutDto.accessToken의 정보로 Authentication 객체 생성

            Long tokenExpiration = jwtTokenService.getTokenExpirationDate(logoutDto.accessToken); //로그아웃 요청한 logoutDto.accessToken의 유효기간

            redisTemplate.delete("RT:" + authentication.getName()); //redis에서 로그아웃한 유저의 refreshToken정보 삭제

            redisTemplate.opsForValue().set(logoutDto.accessToken, "logout", tokenExpiration, TimeUnit.MILLISECONDS); //로그아웃한 유저가 logoutDto.accessToken을 재사용하지 못하도록 로그아웃한 유저의 logoutDto.accessToken 정보를 redis에 등록

            returnValue = ResponseDto.builder().httpStatus(HttpStatus.OK.value()).message("로그아웃 되었습니다.").build();
        } catch (CustomBadRequestException e)
        {
            throw e;
        } catch (Exception e)
    ...
    public ResponseDto updateInfo(UserDto.Info userInfo)
    {
        ...
        try
        {
            User user = userRepository.findByUserId(userInfo.getUserId()).orElseThrow(() -> new CustomBadRequestException("해당 유저가 존재하지 않습니다.")); //변환 대상 유저 조회
