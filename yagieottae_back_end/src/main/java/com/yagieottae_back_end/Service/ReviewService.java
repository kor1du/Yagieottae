...
public class ReviewService
{
    ...
    {
        Boolean isNewRequest = reviewRequestDto.getId() == 0 ? true : false;

        try
        {
            User user = SessionUtil.getUserFromDB();
            Pill pill = pillRepository.findById(reviewRequestDto.getPillId()).orElseThrow(() -> new CustomBadRequestException("약 정보가 존재하지 않습니다."));

            List<Review> reviews = reviewRepository.findByUserIdAndPillId(user.getId(), pill.getId()).orElseThrow(() -> new CustomBadRequestException(pill.getItemName() + "에 작성한 리뷰가 null 입니다!"));

            if (reviews.stream().count() > 2) //유저가 작성한 약 리뷰가 3개 이상
            {
    ...
    //리뷰 조회
    public ResponseDto read(Long pillId)
    {
        try
        {
            List<ReviewDto.Response> reviewList = reviewRepository.findByPillId(pillId).get();
    ...
    public ResponseDto delete(Long reviewId)
    {
        try
        {
            ...
            Review review = reviewRepository.findById(reviewId).orElseThrow(() -> new CustomBadRequestException("해당 리뷰가 존재하지 않습니다!"));

            if (review.getUser().getId() != user.getId()) //리뷰 작성자가 아닌 다른 유저가 삭제 요청시 세션에 저장되어있는 유저 정보 비교
            {
